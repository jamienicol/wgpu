...
8 │   use_me(1);
  │   ^^^^^^^^^
  │
  = note: function `use_me` is annotated with `@must_use` attribute
  = note: use phony assignment to ignore returned value
...
"#,
        r#"error: function annotated with @must_use but has no return type
  ┌─ wgsl:2:2
  │
2 │ @must_use
  │  ^^^^^^^^
